      ...
      {users.map((user) => {
        return (
          <tr key={user.id} className="border-b border-gray-200">
            ...
            <td className="px-4 py-2">{user.publicMetadata.role as string}</td>
            <td className="px-4 py-2">
              <form action={setRole} className="inline">
                <input type="hidden" value={user.id} name="id" />
                <input type="hidden" value="admin" name="role" />
                <button type="submit" className="bg-orange-500 text-white rounded px-2 py-1 mr-2">Make Admin</button>
              </form>
              <form action={setRole} className="inline">
                <input type="hidden" value={user.id} name="id" />
                <input type="hidden" value="student" name="role" />
                <button type="submit" className="bg-green-500 text-white rounded px-2 py-1">Make Student</button>
              </form>
            </td>
          </tr>
        );
