...
export const useUserMetadata = () => {
  const [metadata, setMetadata] = useState<null | { userId: string; role: Roles; } | undefined>(null);

  useEffect(() => {
    const fetchMetadata = async () => {
      const data = getUserMetadata();
      setMetadata(data);
    };
...
export const getUserMetadataUnified = () => {
  if (typeof window !== "undefined") {
    // Client-side
    console.warn("getUserMetadataUnified should not be called directly on the client. Use useUserMetadata hook instead.");
    return null;
  } else {
    // Server-side
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (currentPackage !== null) {
        ...
        const studentPkg = today >= packageStartDate && today <= packageEndDate;
        setHasPackage(studentPkg);
      }
